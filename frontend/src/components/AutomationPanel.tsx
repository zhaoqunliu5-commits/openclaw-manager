...
const TRIGGER_TYPES = [
  { value: 'cron', label: '定时触发', desc: '按固定间隔执行' },
  { value: 'event', label: '事件触发', desc: '当特定事件发生时执行' },
];
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">⚙️ 工作流自动化</h2>
          ...
          >
            <div className="glass-card rounded-2xl p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {TRIGGER_TYPES.map(t => (
                      <button
                        key={t.value}
                        onClick={() => setForm(f => ({ ...f, triggerType: t.value as any }))}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {ACTION_TYPES.map(a => (
                      <button
                        key={a.value}
                        onClick={() => setForm(f => ({ ...f, actionType: a.value as any }))}
                        className={`w-full text-left px-4 py-2.5 rounded-lg border text-sm transition-all ${
              ...
              {form.triggerType === 'cron' && (
                <div>
                  <label className="text-gray-300 text-sm mb-1 block">执行间隔（分钟）</label>
                  <input
                    type="number"
                    min={1}
              ...
              {form.triggerType === 'event' && (
                <div>
                  <label className="text-gray-300 text-sm mb-1 block">事件名称</label>
                  <input
                    value={form.eventName}
                    onChange={e => setForm(f => ({ ...f, eventName: e.target.value }))}
                    placeholder="例：agent.error, service.stopped"
                    className="w-full bg-gray-800/50 border border-gray-600/50 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500"
                  />
                </div>
              )}
              ...
              {form.actionType === 'switch_agent' && (
                <div>
                  <label className="text-gray-300 text-sm mb-1 block">Agent 名称</label>
                  <input
                    value={form.agentName}
                    onChange={e => setForm(f => ({ ...f, agentName: e.target.value }))}
                    placeholder="例：claude, gpt4"
                    className="w-full bg-gray-800/50 border border-gray-600/50 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500"
                  />
                </div>
              )}
              ...
              {form.actionType === 'restart_service' && (
                <div>
                  <label className="text-gray-300 text-sm mb-1 block">服务名称</label>
                  <input
                    value={form.serviceName}
                    onChange={e => setForm(f => ({ ...f, serviceName: e.target.value }))}
                    placeholder="例：openclaw-gateway, canvas"
                    className="w-full bg-gray-800/50 border border-gray-600/50 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500"
                  />
                </div>
              )}
      ...
      {rules.length === 0 ? (
        <div className="glass-card rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-400">还没有自动化规则</p>
          <p className="text-gray-500 text-sm mt-1">点击上方「新建规则」创建你的第一个自动化工作流</p>
        </div>
      ) : (
        <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-400 mt-0.5">
                      ...
                      {rule.lastTriggered && (
                        <span>上次: {new Date(rule.lastTriggered).toLocaleString()}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <motion.button
                    ...
                    className="px-3 py-1.5 bg-gray-500/20 text-gray-300 rounded-lg text-xs hover:bg-gray-500/30 transition-colors"
                  >
                    📋 日志
                  </motion.button>
                  <motion.button
